...
void EndReport(TransState* StateInfo, int terminals)
{
    ...
    for(i=0;i<terminals;i++)
    {
        printf("newOrder:%d, payment:%d, delivery:%d, orderStatus:%d, stockLevel:%d, global_total:%d, global_abort:%d\n",StateInfo[i].NewOrder, StateInfo[i].Payment, StateInfo[i].Delivery, StateInfo[i].Order_status, StateInfo[i].Stock_level, StateInfo[i].global_total, StateInfo[i].global_abort);
    }
